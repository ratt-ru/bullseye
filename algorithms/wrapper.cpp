...
extern "C" {
    void grid_single_pol(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
                         ...
                         visibility_weights_base_type *  visibility_weights, size_t nx, size_t ny, uvw_base_type cell_size_x,
                         uvw_base_type cell_size_y, convolution_base_type * conv, size_t conv_support,
                         size_t conv_oversample, size_t polarization_index, std::complex<grid_base_type> * output_buffer,
                         size_t row_count, const unsigned int * field_array, unsigned int imaging_field,
        ...
        imaging::grid<visibility_base_type,uvw_base_type,
                ...
                (polarization_policy,uvw_transform,convolution_policy,
                 ...
                 flagged_rows,
                 nx,ny,
                 casa::Quantity(cell_size_x,"arcsec"),
                 casa::Quantity(cell_size_y,"arcsec"),
    ...
    void facet_single_pol(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
                          size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
                          const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
    ...
    void grid_4_cor(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
                    ...
                    const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
                    visibility_weights_base_type *  visibility_weights, size_t nx, size_t ny, uvw_base_type cell_size_x,
                    uvw_base_type cell_size_y, convolution_base_type * conv, size_t conv_support,size_t conv_oversample,
                    std::complex<grid_base_type> * output_buffer, size_t row_count, const unsigned int * field_array,
                    unsigned int imaging_field, const unsigned int * spw_index_array) {
        using namespace imaging;
    ...
    void facet_4_cor(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
                     size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
                     const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
                     visibility_weights_base_type *  visibility_weights, size_t facet_nx, size_t facet_ny, uvw_base_type facet_cell_size_x,
                     uvw_base_type facet_cell_size_y, uvw_base_type phase_centre_ra, uvw_base_type phase_centre_dec, const uvw_base_type * facet_centres,
    ...
    void facet_4_cor_corrections(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
				 size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
				 const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
				 visibility_weights_base_type *  visibility_weights, size_t facet_nx, size_t facet_ny, uvw_base_type facet_cell_size_x,
				 uvw_base_type facet_cell_size_y, uvw_base_type phase_centre_ra, uvw_base_type phase_centre_dec, const uvw_base_type * facet_centres,
				 std::size_t num_facet_centres, convolution_base_type * conv, size_t conv_support,size_t conv_oversample,
				 std::complex<grid_base_type> * output_buffer, size_t row_count, const unsigned int * field_array,
				 unsigned int imaging_field, const unsigned int * spw_index_array, std::complex<visibility_base_type> * jones_terms, 
				 bool should_invert_jones_terms, const unsigned int * antenna_1_ids, const unsigned int * antenna_2_ids,
				 const std::size_t * timestamp_ids, std::size_t antenna_count, std::size_t no_timestamps_read, 
    ...
    void grid_single_pol_with_sampling_func(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
					    size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
					    const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
					    visibility_weights_base_type *  visibility_weights, size_t nx, size_t ny, uvw_base_type cell_size_x,
					    uvw_base_type cell_size_y, convolution_base_type * conv, size_t conv_support,
					    size_t conv_oversample, size_t polarization_index, std::complex<grid_base_type> * output_buffer,
					    size_t row_count, const unsigned int * field_array, unsigned int imaging_field,
					    const unsigned int * spw_index_array, std::complex<grid_base_type> * sampling_function_buffer) {
        using namespace imaging;
        ...
        polarization_gridding_policy_type polarization_policy(phase_transform,
                ...
                channel_count);
        convolution_policy_type convolution_policy(nx,ny,conv_support,conv_oversample,
                conv, polarization_policy,sampling_function_buffer);
    ...
    void facet_single_pol_with_sampling_func(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
					     size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
					     const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
					     visibility_weights_base_type *  visibility_weights, size_t facet_nx, size_t facet_ny, uvw_base_type facet_cell_size_x,
					     uvw_base_type facet_cell_size_y, uvw_base_type phase_centre_ra, uvw_base_type phase_centre_dec, const uvw_base_type * facet_centres,
					     std::size_t num_facet_centres, convolution_base_type * conv, size_t conv_support,size_t conv_oversample,size_t polarization_index,
					     std::complex<grid_base_type> * output_buffer, size_t row_count, const unsigned int * field_array,
					     unsigned int imaging_field, const unsigned int * spw_index_array, std::complex<grid_base_type> * sampling_function_buffer) {
        using namespace imaging;
        size_t no_facet_pixels = facet_nx*facet_ny;
        for (size_t facet_index = 0; facet_index < num_facet_centres; ++facet_index) {
            uvw_base_type new_phase_ra = facet_centres[2*facet_index];
            ...
            convolution_policy_type convolution_policy(facet_nx,facet_ny,
                    conv_support,conv_oversample,
                    conv, polarization_policy,sampling_function_buffer + no_facet_pixels*facet_index);
            imaging::grid<visibility_base_type,uvw_base_type,
                    reference_wavelengths_base_type,convolution_base_type,
                    visibility_weights_base_type,grid_base_type,
                    baseline_transform_policy_type,
    ...
    void grid_4_cor_with_sampling_func(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
				       size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
				       const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
				       visibility_weights_base_type *  visibility_weights, size_t nx, size_t ny, uvw_base_type cell_size_x,
				       uvw_base_type cell_size_y, convolution_base_type * conv, size_t conv_support,size_t conv_oversample,
    ...
    void facet_4_cor_with_sampling_func(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
					size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
					const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
					visibility_weights_base_type *  visibility_weights, size_t facet_nx, size_t facet_ny, uvw_base_type facet_cell_size_x,
					uvw_base_type facet_cell_size_y, uvw_base_type phase_centre_ra, uvw_base_type phase_centre_dec, const uvw_base_type * facet_centres,
        ...
        for (size_t facet_index = 0; facet_index < num_facet_centres; ++facet_index) {
            ...
            convolution_policy_type convolution_policy(facet_nx,facet_ny,
                    conv_support,conv_oversample,
                    conv, polarization_policy, sampling_function_buffer + no_facet_pixels*facet_index); //only one sampling function over all 4 polarizations
            imaging::grid<visibility_base_type,uvw_base_type,
                    reference_wavelengths_base_type,convolution_base_type,
                    visibility_weights_base_type,grid_base_type,
                    baseline_transform_policy_type,
    ...
    void facet_4_cor_corrections_with_sampling_func(const std::complex<visibility_base_type> * visibilities, const imaging::uvw_coord<uvw_base_type> * uvw_coords,
						    size_t timestamp_count, size_t baseline_count, size_t channel_count, size_t number_of_polarization_terms,
						    const reference_wavelengths_base_type * reference_wavelengths, const bool * flags, const bool * flagged_rows,
						    visibility_weights_base_type *  visibility_weights, size_t facet_nx, size_t facet_ny, uvw_base_type facet_cell_size_x,
						    uvw_base_type facet_cell_size_y, uvw_base_type phase_centre_ra, uvw_base_type phase_centre_dec, const uvw_base_type * facet_centres,
						    ...
						    bool should_invert_jones_terms, const unsigned int * antenna_1_ids, const unsigned int * antenna_2_ids,
						    const std::size_t * timestamp_ids, std::size_t antenna_count, std::size_t no_timestamps_read, 
						    std::size_t spw_count, std::complex<grid_base_type> * sampling_function_buffer) {
        using namespace imaging;
        assert(number_of_polarization_terms == 4); //Only supports 4 correlation visibilties in this mode
